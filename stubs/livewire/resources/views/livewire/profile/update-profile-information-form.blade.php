<section>
    ...
    <form wire:submit="updateProfileInformation" class="mt-6 space-y-6">
        <div>
            <x-input label="Avatar" for="avatar" />
            @if (auth()->user()->avatar)
                <img src="{{ Storage::url(auth()->user()->avatar) }}" alt="{{ auth()->user()->name }}"
                    class="w-20 h-20 rounded-full object-cover mt-2" />
            @else
                <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&color=7F9CF5&background=F0F0FF&size=80"
                    alt="{{ auth()->user()->name }}" class="w-20 h-20 rounded-full object-cover mt-2" />
            @endif
            <input type="file" wire:model.live="avatar" id="avatar"
            ...
            @if ($avatar)
                <img src="{{ $avatar->temporaryUrl() }}" class="w-20 h-20 rounded-full object-cover mt-2" />
            @endif
            @error('avatar')
